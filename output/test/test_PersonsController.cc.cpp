...
void PersonsController::get(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback, const std::string &sort_field, const std::string &sort_order) const {
    LOG_DEBUG << "get";
...
void PersonsController::getOne(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback, int personId, const std::string &sort_field, const std::string &sort_order) const {
    ...
    mp.find(personId,
      [callbackPtr](const std::size_t count)
      {
          if (count != 1) {
              ...
              PersonDetails personDetails{department};
              auto ret = personDetails.toJson();
              auto resp = HttpResponse::newHttpJsonResponse(ret);
              resp->setStatusCode(HttpStatusCode::k200OK);
              (*callbackPtr)(resp);
          }
